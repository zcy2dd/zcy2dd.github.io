...
- Client
  ...
  - 在获得Region的存储位置信息后，直接从Region Server上读取数据
- 一个Master Server
...
- 单个StoreFile过大时，会触发split操作（即把当前的region分成2个region？）
...
- 用户更新数据必须首先写入日志后，才能写入MemStore缓存
- 出现故障
...
7. Region Split
   ...
   - **分裂过程中，被分裂的Region会暂停读写服务**。父Region的数据文件并不会真正的分裂，而是仅仅通过更改引用方式，来实现快速分裂，
